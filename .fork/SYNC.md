...
- **Upstream baseline:** stable tag `v1.42.0` (commit `c898919d0118`)
- **Last synced upstream release:** v1.42.0 (2026-09-09)
...
| File | Change |
|---|---|
| `extensions/reddit/.../patches/NsfwFeedModePatch.java` | **New file.** `filterHomeFeedResponse(Object)` injection point, and nothing else. Reads `Settings.NSFW_FEED_MODE` live (it is a mode, not a startup flag). Keeps **only** edges recognised as 18+ posts: the home feed ships 20+ non-post element types (four kinds of community recommendation carousel, chat channel units, topic pills, taxonomy rows, explore features), none of which carries a `t3_` id, and keeping them was what put SFW content in an 18+ feed. Fails open when **no** edge on a page carries a post id, so a model change leaves the feed unfiltered instead of empty, and holds one post back when a page would otherwise be left with nothing to draw - the only way an SFW post reaches the feed now. |
...
| `extensions/reddit/.../nsfw/NsfwCellScanner.java` | **New file.** Android-free. Reads a post's id and NSFW state out of Reddit's GraphQL cell models by **shape, not name**: the post id is the `String` holding a `t3_` fullname (self-validating), and NSFW is the `NSFW` constant of an enum that **tags posts**, recognised by the company it keeps - the same type must also declare `ORIGINAL`, `QUARANTINED` and `SPOILER`, which picks out exactly `com.reddit.type.CellIndicatorType` and `com.reddit.feeds.model.IndicatorType`. Matching the constant name alone is not enough: **eleven** enums in the app declare an `NSFW` constant, among them `NSFWState` (a subreddit's own rating), `DisplayTag`, `QueryTag` and `MediaBlurType`, and any of those reachable from a post's edge marked it 18+. Depth- and cycle-bounded walk, deep enough for the nine hops from an edge to its indicators. |
| `extensions/reddit/src/test/.../nsfw/NsfwCellScannerSelfTest.java` | **New file.** Plain-javac self-test; must print `24 passed, 0 failed`. Builds an edge with the real feed's nine-level shape, so a walk that is too shallow to reach the indicators fails the test rather than the feed. |
